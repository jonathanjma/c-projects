...
cache_t *make_cache(int capacity, int block_size, int assoc, enum protocol_t protocol, bool lru_on_invalidate_f){
  ...
  cache->n_tag_bit = 32 - log2(capacity) + log2(assoc);
...
unsigned long get_cache_tag(cache_t *cache, unsigned long addr) {
  unsigned long tag_bits = addr >> (cache->n_offset_bit + cache->n_index_bit);
  unsigned long tag_mask = (1 << cache->n_tag_bit) - 1;
  return tag_bits & tag_mask;
}
...
bool access_cache(cache_t *cache, unsigned long addr, enum action_t action) {
  ...
  int lru_way = 0; // least recently used
